...
sub lisp_read
{
	...
	if (/\G\s*;+([^\n]*)/gc) {
		...
		if (ref($form) eq "ARRAY" && @$form == 0) {
		    ...
		    $stack[-1][-1] = my_symbol("()"); # my_symbol("nil");
		}
		$form = pop(@stack);
		last if $one && !@stack;
	    }
		 ...
		 )$/gcxs)                      # no end quote
	{
	    ...
	    # $str =~ s/((?:\\[A-Z]-)*\\(?:\^.|[0-7]{1,3}|.))/
	    #          chr(parse_char($1,1))/ge;
	    print "${indent}UNCLOSED STRING $str\n" if $DEBUG;
	    push(@$form, $str);
	    last if $one && !@stack;
	} elsif (/\G\s*\'/gc) {
	    print "${indent}QUOTE\n" if $DEBUG;
	    ...
	    my($subform, $pos);
	    ($subform, $pos, $err) =
		lisp_read(substr($_, $old_pos), 1, $level+1, $rptErrs);
	    pos($_) = $old_pos + $pos;
    ...
    if ( wantarray )
    {
	return $rptErrs ? ($form, pos($_), $err) : ($form, pos($_));
    }
    else
    {
	return $form;
